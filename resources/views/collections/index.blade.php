    ...
    @if (count($collections) > 0)
                    ...
                    <table id="main-table" class="text-center table table-bordered table-hover table-striped table-condensed">
                        <thead>
                            <tr>
                                ...
                                <th onclick="sortTable(1)" >القسم</th>
                                <th onclick="sortTable(2)" >اسم {{ $page_ar }}</th>
                                <th onclick="sortTable(3)" >تاريخ الإنشاء</th>
                                <th onclick="sortTable(4)" >تاريخ التعديل</th>                       
                                <td>تحكم</td>
                            </tr>
                        </thead>
                        <tbody>
                            @forelse ($collections as $collection)
                                <tr>
                                    <td>{{ $collection->id }}</td>
                                    <td>{{ $collection->section['name'] }}</td>
                                    <td>{{ $collection->name }}</td>
                                    <td>{{ $collection->created_at }}</td>
                                    <td>{{ $collection->updated_at }}</td>
                                    <td>
                                        <a href="/{{ $pages }}/{{ $collection->id }}" class="btn btn-info"><i class="fa fa-eye"></i> التفاصيل </a>
                                        <a href="/{{ $pages }}/{{ $collection->id }}/edit" class="btn btn-success"><i class="fa fa-edit"></i> تعديل </a>
                                        <button type="button" 
                                                data-sender="/{{ $pages }}/{{ $collection->id }}" 
        ...
        <div class="panel-footer">
            <div class="pull-right page-info">
                {{ $collections->links('includes.pagination') }}
            </div>
            <div class="pull-left page-info">
                <span class="row-count">عدد السجلات : {{ $collections->count() }}</span>                                
            </div>
        </div>
    @else 
